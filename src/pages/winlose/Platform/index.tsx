                ...
                {format.map((format, _, arr) => {
                  return (
                    <ExpandedDetail
                      ...
                      getFileName={(params) => `${records.platform}-${format}(${dayjs(params.startDate).format('YYYYMMDD')}-${dayjs(params.endDate).format('YYYYMMDD')})`}
                      expander={() => {
                        if (arr.length === 1) {
                          expander()
                        }
        ...
        <DialogPanel className='max-w-none w-fit'>
          {detailDialog && (
            <ExpandedDetail
              ...
              getText={(params) => (
                <>
                  <Text className='text-xl'>{detailDialog.platform} {detailDialog.format} For {detailDialog.currency || detailDialog.game}</Text>
              ...
              getFileName={(params) => `${detailDialog.platform}-${detailDialog.format}-For-${detailDialog.currency || detailDialog.game}(${dayjs(params.startDate).format('YYYYMMDD')}-${dayjs(params.endDate).format('YYYYMMDD')})`}
              ...
              filterParams={{
                ...
                platform: detailDialog.platform
              }}
            />
          )}
        </DialogPanel>
